...
$extraArgs = @("/DumpConfigToFiles", $XmlDir)

if ($Update) {
    $extraArgs += @("-update", "-force")
    Write-Host "Инкрементальная выгрузка конфигурации в $XmlDir..."
} else {
    Write-Host "Полная выгрузка конфигурации в $XmlDir..."
}
